...
if ( ! class_exists( 'Elearning_Education_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Elearning_Education_WPTT_WebFont_Loader {
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $elearning_education_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $elearning_education_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$elearning_education_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$elearning_education_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$elearning_education_font_family = trim( str_replace( array( "'", ';' ), '', $elearning_education_font_family ) );
					$elearning_education_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $elearning_education_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $elearning_education_font_family ] ) ) {
					$result[ $elearning_education_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$elearning_education_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$elearning_education_font_family_url = $this->get_absolute_path( $elearning_education_font_family_url );

					$result[ $elearning_education_font_family ][] = $elearning_education_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $elearning_education_font_family ] = array_flip( array_flip( $result[ $elearning_education_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Elearning_Education_WPTT_WebFont_Loader( $url );
